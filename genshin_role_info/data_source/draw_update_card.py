...
from ..utils.card_utils import avatar_path, bg_path, get_font, json_path, other_path, weapon_path
...
async def draw_role_pic(uid: str, role_dict: dict | list, player_info):
    ...
    role_list = sorted(
        ...
        key=lambda x: (
            ...
            -["火", "水", "草", "雷", "冰", "风", "岩"].index(player_info.get_roles_info(x)["元素"]),
        ),
        reverse=True,
    )
    # 绘制角色卡片
    for index, role in enumerate(role_list):
        ...
        draw_center_text(card_bg_draw, f"{data['武器']['精炼等级']}", card_size[0] + 1 * multiple - 60, card_size[0] + 1 * multiple, card_size[1] + 1 * multiple - 80, "white", get_font(30 * multiple, "优设标题黑.ttf"))
