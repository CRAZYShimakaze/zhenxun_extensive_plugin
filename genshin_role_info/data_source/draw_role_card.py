...
async def draw_role_card(uid, data, player_info, plugin_version, only_cal, title=None):
    ...
    if not only_cal:
        ...
        if total_cnt and total_all <= 66 * total_cnt:
            ...
            '''
            total_rank = 'ACE' if total_all > 66 * 5 else 'ACE' if total_all > 56.1 * 5 else 'ACE' if total_all > 49.5 * 5 else 'SSS' if total_all > 42.9 * 5 else 'SS' if total_all > 36.3 * 5 else 'S' if total_all > 29.7 * 5 else 'A' if total_all > 23.1 * 5 else 'B' if total_all > 16.5 * 5 else 'C' if total_all > 10 * 5 else 'D'
        else:
            total_rank = 'D'
