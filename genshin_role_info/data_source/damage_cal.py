...
upheaval_value = [8.6, 9.3, 10.0, 10.6, 11.3, 12.3, 13.3, 14.4, 15.7, 17.1, 18.6, 20.3, 22.2, 24.3, 26.9, 29.5, 32.2, 34.9, 37.6, 40.3, 43.1, 45.9, 48.6, 51.4, 54.2,
                  56.6, 59.1, 61.5, 64.9, 68.2, 71.3, 74.5, 77.7, 80.9, 84.6, 88.3, 92, 95.9, 99.8, 103.7, 107.7, 112.1, 116.8, 121.1, 128.0, 134.3, 140.8, 147.5, 154.5,
                  161.8, 168.4, 175.3, 182.2, 189.3, 199.3, 208.2, 217.2, 226.5, 236.3, 246.4, 256.8, 269.6, 282.8, 296.3, 312.2, 325.7, 339.8, 353.9, 368.3, 382.8,
                  397.4, 412.3, 425.6, 438.9, 457.1, 473.4, 489.7, 505.6, 522.4, 538.7, 555.0, 571.5, 588.2, 605.1, 626.9, 644.5, 622.7, 681.7, 702.6, 723.4]
...
def get_role_dmg(data: dict):
    ...
    if data['名称'] == '钟离':
        ...
        dmg_data['原岩共鸣'] = udc(dm['原岩共鸣'] * attack + (health * 0.019 if level_role > 60 else 0), (cr + ve['暴击率'], cd), db[6] + ve['增伤'], level_role, rcd=0.2)
        dmg_data['天星伤害'] = udc(dm['天星'] * attack + (health * 0.33 if level_role > 60 else 0), (cr + vq['暴击率'], cd), db[6] + vq['增伤'], level_role, rcd=0.2)
        a = udc(dm['踢枪'] * attack + (health * 0.0139 if level_role > 60 else 0), (cr + va['普攻暴击率'], cd), db[0] + va['普攻增伤'], level_role, rcd=0.2)
        if data['武器']['名称'] == '流月针':
        ...
        dmg_data['梦想一刀'] = udc((dm['梦想一刀基础'] + dm['梦想一刀愿力']) * attack, (cr + vq['暴击率'], cd), db[2] + vq['增伤'] + weapon_buff, level_role, dci=dci)
        dmg_data['梦想一刀超激化'] = udc((dm['梦想一刀基础'] + dm['梦想一刀愿力']) * attack + r, (cr + vq['暴击率'], cd), db[2] + vq['增伤'], level_role, dci=dci)
        a1 = udc((dm['梦想一心重击基础'][0] + dm['梦想一心愿力']) * attack, (cr + vq['暴击率'], cd), db[2] + vq['增伤'], level_role, dci=dci)
        a2 = udc((dm['梦想一心重击基础'][1] + dm['梦想一心愿力']) * attack, (cr + vq['暴击率'], cd), db[2] + vq['增伤'], level_role, dci=dci)
        ...
        dmg_data['大招持续伤害'] = udc(dm['大招持续'] * attack, (cr + vq['暴击率'], cd), db[-1] + vq['增伤'], level_role, rcd=dm['大招减抗'])
    elif data['名称'] == '珊瑚宫心海':
        health = data['属性']['基础生命'] + data['属性']['额外生命']
        ...
        dmg_data['开大普攻治疗量'] = (
            str(int((float(dm['大招治疗量'][0].replace('%生命值上限', '')) / 100.0 * health + float(dm['大招治疗量'][1])) * (1 + data['属性']['治疗加成']))),)
        dmg_data['战技治疗量'] = (
            str(int((float(dm['水母治疗量'][0].replace('%生命值上限', '')) / 100.0 * health + float(dm['水母治疗量'][1])) * (1 + data['属性']['治疗加成']))),)
    else:
...
def get_dmg_data(data, dm, va, ve, vq):
    ...
    v = {'A': {'暴击率': va['普攻暴击率'], '暴击伤害': 0, '增伤': va['普攻增伤'], '额外倍率': va['普攻额外倍率'], '减抗': va['减抗'], '减防': 0},
         'AZ': {'暴击率': va['重击暴击率'], '暴击伤害': 0, '增伤': va['重击增伤'], '额外倍率': va['重击额外倍率'], '减抗': va['减抗'], '减防': 0},
         'AX': {'暴击率': va['下落攻击暴击率'], '暴击伤害': 0, '增伤': va['下落攻击增伤'], '额外倍率': va['下落攻击额外倍率'], '减抗': va['减抗'], '减防': 0},
         'E': {'暴击率': ve['暴击率'], '暴击伤害': 0, '增伤': ve['增伤'], '额外倍率': ve['额外倍率'], '减抗': ve['减抗'], '减防': 0},
         'Q': {'暴击率': vq['暴击率'], '暴击伤害': 0, '增伤': vq['增伤'], '额外倍率': 0, '减抗': vq['减抗'], '减防': 0}, }
    dmt = {'攻击力': data['属性']['基础攻击'] + data['属性']['额外攻击'], '生命值': data['属性']['基础生命'] + data['属性']['额外生命'],
    ...
    for name, num in dm.items():
        ...
        if skill_type == 'B':
            ...
            if len(para) != 1:
                for p in para[1:]:
                    if p.startswith('r'):
                        r = growth_reaction(data['属性']['元素精通'], float(p[3:])) if f'{p[1:3]}系数' not in data['属性'] else growth_reaction(data['属性']['元素精通'],
                                                                                                                                                float(p[3:]),
                                                                                                                                                data['属性'][
                                                                                                                                                    f'{p[1:3]}系数'])
                    if p.startswith('j'):
            ...
            if isinstance(num, tuple):
                n1 = udc(num[0] * dmt[t] + v[skill_type]['额外倍率'] + j, (cr + v[skill_type]['暴击率'], cd + v[skill_type]['暴击伤害']), db[e] + v[skill_type]['增伤'],
                         level_role, r=r, rcd=v[skill_type]['减抗'], dcr=v[skill_type]['减防'])
                n2 = udc(num[1] * dmt[t] + v[skill_type]['额外倍率'] + j, (cr + v[skill_type]['暴击率'], cd + v[skill_type]['暴击伤害']), db[e] + v[skill_type]['增伤'],
                         level_role, r=r, rcd=v[skill_type]['减抗'], dcr=v[skill_type]['减防'])
                dmg_data[skill_name] = (n1[0] + '+' + n2[0], n1[1] + '+' + n2[1])
            else:
                if n == '1':
                    dmg_data[skill_name] = udc(num * dmt[t] + v[skill_type]['额外倍率'] + j, (cr + v[skill_type]['暴击率'], cd + v[skill_type]['暴击伤害']),
                                               db[e] + v[skill_type]['增伤'], level_role, r=r, rcd=v[skill_type]['减抗'], dcr=v[skill_type]['减防'])
                else:
                    dmg = udc(num * dmt[t] + v[skill_type]['额外倍率'] + j, (cr + v[skill_type]['暴击率'], cd + v[skill_type]['暴击伤害']), db[e] + v[skill_type]['增伤'],
                              level_role, r=r, rcd=v[skill_type]['减抗'], dcr=v[skill_type]['减防'])
                    dmg_data[skill_name] = (dmg[0] + '*' + n, dmg[1] + '*' + n)
    return dmg_data


def udc(dm: float, crit: Tuple[float, float], db: float, sl: int, rcb: Optional[float] = 0.1, rcd: Optional[float] = 0, el: Optional[int] = 90, dcr: Optional[float] = 0,
        dci: Optional[float] = 0, r: Optional[float] = 1, ) -> List[str]:
    """
    计算伤害
    :param dm: 倍率区
    :param crit: 暴击区
...
def weapon_common_fix(data: dict):
    ...
    weapon = data['武器']
    # 针对q的额外属性
    extra_q = {'暴击率': 0, '增伤': 0, '减抗': 0}
    # 针对e的额外属性
    extra_e = {'暴击率': 0, '增伤': 0, '额外倍率': 0, '减抗': 0}
    # 针对a的额外属性
    extra_a = {'普攻暴击率': 0, '普攻增伤': 0, '普攻额外倍率': 0, '重击暴击率': 0, '重击增伤': 0, '重击额外倍率': 0, '下落攻击暴击率': 0, '下落攻击增伤': 0,
               '下落攻击额外倍率': 0, '减抗': 0}
    # 单手剑
    if weapon['名称'] == '波乱月白经津':
...
def get_damage_multipiler(data: dict) -> dict:
    ...
    dm = {}
    if data['名称'] == '钟离':
        return {'玉璋护盾': (float(skill_data['元素战技·地心']['数值']['护盾附加吸收量'][level_e].replace('%最大生命值', '')) / 100.0,
    ...
    if data['名称'] == '胡桃':
        ...
        dm['B:l1-攻击力'] = (float(skill_data['蝶引来生']['数值']['攻击力提高'][level_e].replace('%生命值上限', '')) / 100.0 * (health), '开E后')
    ...
    if data['名称'] == '雷电将军':
        ...
        return {'协同攻击': float(skill_data['神变·恶曜开眼']['数值']['协同攻击伤害'][level_e].replace('%', '')) / 100.0,
                'e增伤': float(skill_data['神变·恶曜开眼']['数值']['元素爆发伤害提高'][level_e].replace('每点元素能量', '').replace('%', '')) / 100.0 * 90,
                '梦想一刀基础': float(skill_data['奥义·梦想真说']['数值']['梦想一刀基础伤害'][level_q].replace('%', '')) / 100.0,
                '梦想一刀愿力': float(skill_data['奥义·梦想真说']['数值']['愿力加成'][level_q].split('%/')[0].replace('每层', '')) / 100.0 * 60,
                '梦想一心重击基础': (float(qa[0].replace('%', '')) / 100.0, float(qa[1].replace('%', '')) / 100.0),
                '梦想一心愿力': float(skill_data['奥义·梦想真说']['数值']['愿力加成'][level_q].split('%/')[1].replace('%攻击力', '')) / 100.0 * 60,
                '梦想一心能量': float(skill_data['奥义·梦想真说']['数值']['梦想一心能量恢复'][level_q])}
    if data['名称'] == '魈':
        a = skill_data['普通攻击·卷积微尘']['数值']['低空/高空坠地冲击伤害'][level_a].split('/')
        return {'B:l1-增伤-AX': (float(skill_data['靖妖傩舞']['数值']['普通攻击/重击/下落攻击伤害提升'][level_q].replace('%', '')) / 100,),
                'E-e风:风轮两立': float(skill_data['风轮两立']['数值']['技能伤害'][level_e].replace('%', '')) / 100.0,
                'AX-e风:低空下落首戳': float(a[0].replace('%', '')) / 100, 'AX-e风:高空下落首戳': float(a[1].replace('%', '')) / 100, }
    if data['名称'] == '香菱':
        return {'B:c1-减抗-*': (0.15, '锅巴减抗'), 'E-e火:锅巴喷火': float(skill_data['锅巴出击']['数值']['喷火伤害'][level_e].replace('%', '')) / 100.0,
    ...
    if data['名称'] == '申鹤':
        return {'冰翎': float(skill_data['仰灵威召将役咒']['数值']['伤害值提升'][level_e].replace('%', '')) / 100.0,
    ...
    if data['名称'] == '刻晴':
        ...
        return {'B:l70-暴击率-*': (0.15,), 'B:c6-增伤-*': (0.24, '六命满层'), 'B:c4-攻击力': (data['属性']['基础攻击'] * 0.25, '四命触发'),
    ...
    if data['名称'] == '可莉':
        return {'B:l50-增伤-AZ': (0.5, '砰砰礼物触发'), 'B:c2-减防-*': (0.23, '二命减防'), 'B:c6-增伤-*': (0.1,),
    ...
    if data['名称'] == '八重神子':
        ...
        return {'B:l70-增伤-E': (data['属性']['元素精通'] * 0.0015,), 'B:c4-增伤-*': (0.2,), 'B:c6-减防-E': (0.6,),
    ...
    if data['名称'] == '阿贝多':
        return {'E-t防御力-e岩:阳华绽放': float(skill_data['创生法·拟造阳华']['数值']['刹那之花伤害'][level_e].replace('%防御力', '')) / 100.0,
                'Q-e岩:大招首段': float(skill_data['诞生式·大地之潮']['数值']['爆发伤害'][level_q].replace('%', '')) / 100.0}
    if data['名称'] == '神里绫华':
        return {'B:l50-增伤-AZ': (0.3,), 'B:l70-增伤-*': (0.18,), 'B:c4-减防-*': (0.3,), 'B:c6-增伤-AZ': (2.98, '满命触发'),
    ...
    if data['名称'] == '行秋':
        ...
        return {'B:l70-增伤-*': (0.2,), 'B:c2-减抗-*': (0.15,), 'E-e水:画雨笼山': (float(e[0].replace('%', '')) / 100.0 * dm, float(e[1].replace('%', '')) / 100.0 * dm),
                'Q-e水:裁雨留虹每段': float(skill_data['古华剑·裁雨留虹']['数值']['剑雨伤害'][level_e].replace('%', '')) / 100.0}
    if data['名称'] == '夜兰':
        return {'B:d': ['不计算天赋增伤'], 'B:l50-生命值': (data['属性']['基础生命'] * 0.18, '天赋按3元素'),
    ...
    if data['名称'] == '优菈':
        ...
        return {'B:c1-增伤-*': (0.3,), 'B:c4-增伤-Q': (0.25, '四命触发'),
    ...
    if data['名称'] == '达达利亚':
        ...
        return {'E-e水:近战重击': (float(e[0].replace('%', '')) / 100.0, float(e[1].replace('%', '')) / 100.0),
    ...
    if data['名称'] == '迪卢克':
        return {'B:l70-增伤-*': (0.2,), 'B:c4-增伤-E': (0.4, '四命增伤'), 'B:c2-攻击力': (data['属性']['基础攻击'] * 0.3, '二命满层'), 'B:c1-增伤-*': (0.15, '一命增伤'),
    ...
    if data['名称'] == '凝光':
        return {'B:l70-增伤-*': (0.12, '穿屏增伤'), 'AZ-e岩:重击': float(skill_data['普通攻击·千金掷']['数值']['重击伤害'][level_a].replace('%', '')) / 100.0,
    ...
    if data['名称'] == '菲谢尔':
        ...
        return dm
    if data['名称'] == '北斗':
        return {'B:c6-减抗-*': (0.15, '六命减抗'), 'E-e雷:完美弹反': float(skill_data['捉浪']['数值']['基础伤害'][level_e].replace('%', '')) / 100.0 + 2 * float(
    ...
    if data['名称'] == '诺艾尔':
        ...
        return {'B:l1-攻击力': ((float(skill_data['大扫除']['数值']['攻击力提高'][level_q].replace('%防御力', '')) / 100.0 + extra) * defense,),
                ...
                'T:普攻治疗量/概率': str(int((float(ez[0].replace('%防御力', '')) / 100.0 * defense + float(ez[1])) * (1 + data['属性']['治疗加成']))) + '/' +
                                     skill_data['护心铠']['数值']['治疗触发几率'][level_e]}
    if data['名称'] == '神里绫人':
        n = 2 if data['等级'] >= 50 else 0
        return {'B:c1-增伤-A': (0.4, '一命增伤'),
                'B:l1-额外倍率-A': (float(skill_data['神里流·镜花']['数值']['浪闪伤害值提高'][level_e].replace('%最大生命值/层', '')) / 100.0 * n * (health),),
    ...
    if data['名称'] == '荒泷一斗':
        return {'B:d': ['开启大招'], 'B:l70-额外倍率-AZ': (0.35 * (data['属性']['基础防御'] + data['属性']['额外防御']),), 'B:c6-暴击伤害-AZ': (0.7,), 'B:l1-攻击力': (
            float(skill_data['最恶鬼王·一斗轰临！！']['数值']['攻击力提高'][level_q].replace('%防御力', '')) / 100.0 * (
    ...
    if data['名称'] == '宵宫':
        return {'B:l50-增伤-*': (0.2, '被动一满层'), 'B:c2-增伤-*': (0.25, '二命触发'),
                'A-e火-n2:普攻第一段': float(skill_data['焰硝庭火舞']['数值']['炽焰箭伤害'][level_e].replace('%普通攻击伤害', '')) / 100.0 * float(
                    skill_data['普通攻击·烟火打扬']['数值']['一段伤害'][level_a].replace('%*2', '')) / 100.0,
                'A-e火-r蒸发1.5:普攻第三段蒸发': (float(skill_data['焰硝庭火舞']['数值']['炽焰箭伤害'][level_e].replace('%普通攻击伤害', '')) / 100.0) * float(
    ...
    if data['名称'] == '烟绯':
        ...
        return {'B:l50-增伤-*': (0.05 * max_, '满层丹火印'), 'B:c2-暴击率-AZ': (0.2,),
                'B:l1-增伤-AZ': (float(skill_data['凭此结契']['数值']['重击伤害提升'][level_q].replace('%', '')) / 100.0,), 'AZ-e火:满丹火印重击': AZ,
    ...
    if data['名称'] == '珊瑚宫心海':
        return {'普攻第一段': float(skill_data['普通攻击·水有常形']['数值']['一段伤害'][level_a].replace('%', '')) / 100.0,
    ...
    if data['名称'] == '枫原万叶':
        ...
        return {'B:d': ['技能仅计算风伤部分'], 'B:c6-增伤-AX': (data['属性']['元素精通'] * 0.002,),
    ...
    if data['名称'] == '鹿野院平藏':
        ...
        up = data['属性']['扩散系数'] if '扩散系数' in data['属性'] else 0
        return {'B:c6-暴击率-E': (0.16,), 'B:c6-暴击伤害-E': (0.32,),
                'AZ-e风:重击': float(skill_data['普通攻击·不动流格斗术']['数值']['重击伤害'][level_a].replace('%', '')) / 100.0, 'E-e风:满层勠心拳': (float(
                e['技能伤害'][level_e].replace('%', '')) + 4 * float(e['变格伤害提升'][level_e].replace('%/层', '')) + float(
                e['正论伤害提升'][level_e].replace('%', ''))) / 100.0,
                'Q-e风:聚风蹴真空弹': float(skill_data['聚风蹴']['数值']['不动流·真空弹伤害'][level_q].replace('%', '')) / 100.0,
                'T:扩散伤害': int(upheaval_reaction(data['等级'], '扩散', data['属性']['元素精通'], up))}
    if data['名称'] == '班尼特':
        attack_increase = float(skill_data['美妙旅程']['数值']['攻击力加成比例'][level_q].replace('%', '')) / 100.0 + (0.2 if len(data['命座']) >= 1 else 0)
        hp_recover = skill_data['美妙旅程']['数值']['持续治疗'][level_q].split('+')
        return {'B:c6-增伤-E': (0.15,), 'B:l1-攻击力': (int(attack_increase * data['属性']['基础攻击']),),
                ...
                'T:大招攻击加成': int(attack_increase * data['属性']['基础攻击']), 'T:大招持续治疗': int(
                (float(hp_recover[0].replace('每秒', '').replace('%生命值上限', '')) / 100.0 * health + int(hp_recover[1])) * (1 + data['属性']['治疗加成'])), }
    if data['名称'] == '温迪':
        up = data['属性']['扩散系数'] if '扩散系数' in data['属性'] else 0
        return {'B:d': ['技能仅计算风伤部分'], 'B:c2-减抗-*': (0.24, '二命减抗'), 'B:c4-增伤-*': (0.25, '四命增伤'), 'B:c6-减抗-*': (0.2, '六命减抗'),
    ...
    if data['名称'] == '莫娜':
        if len(data['命座']) >= 1:
            if '蒸发系数' in data['属性']:
                ...
                data['属性']['蒸发系数'] = 0.15
        return {'B:c4-暴击率-*': (0.15,), 'B:c6-增伤-AZ': (1.8, '六命重击增伤满层'),
    ...
    if data['名称'] == '琴':
        ...
        return {'B:c1-增伤-E': (0.4, '战技长按1秒'), 'B:c4-减抗-E': (0.4, '四命减抗'), 'B:c4-减抗-Q': (0.4,),
                ...
                'T:大招瞬时治疗': int((float(recovery1[0].replace('%攻击力', '')) / 100.0 * attack + int(recovery1[1])) * (1 + data['属性']['治疗加成'])),
                'T:大招持续治疗': int(
                    (float(recovery2[0].replace('每秒', '').replace('%攻击力', '')) / 100.0 * attack + int(recovery2[1])) * (1 + data['属性']['治疗加成'])), }
    if data['名称'] == '七七':
        ...
        return {'B:c1-增伤-AZ': (0.15,), 'AZ:重击': (float(a[0].replace('%', '')) / 100.0, float(a[0].replace('%', '')) / 100.0),
                'E-e冰:E释放伤害': float(skill_data['仙法·寒病鬼差']['数值']['技能伤害'][level_e].replace('%', '')) / 100.0,
                'T:开E后普攻治疗量': int((float(a_rec[0].replace('%攻击力', '')) / 100.0 * attack + int(a_rec[1])) * (1 + data['属性']['治疗加成'])),
                'T:E持续治疗量': int((float(e_rec[0].replace('%攻击力', '')) / 100.0 * attack + int(e_rec[1])) * (1 + data['属性']['治疗加成'])),
                'T:挂符每次治疗量': int((float(q_rec[0].replace('%攻击力', '')) / 100.0 * attack + int(q_rec[1])) * (1 + data['属性']['治疗加成'])),
    ...
    if data['名称'] == '提纳里':
        ...
        return {'B:c1-暴击率-AZ': (0.15,), 'B:c2-增伤-*': (0.2, '二命增伤'), 'B:c4-增伤-*': (0, '四命精通'),
                'B:l70-增伤-AZ': (min(data['属性']['元素精通'], 1000) * 0.0006,), 'B:l70-增伤-Q': (min(data['属性']['元素精通'], 1000) * 0.0006,),
    ...
    if data['名称'] == '柯莱':
        ...
        return {'E-e草:飞叶轮': float(skill_data['拂花偈叶']['数值']['技能伤害'][level_e].replace('%', '')) / 100.0,
    ...
    if data['名称'] == '多莉':
        ...
        return {'E-e雷:断除烦恼炮': float(skill_data['镇灵之灯·烦恼解决炮']['数值']['断除烦恼炮伤害'][level_e].replace('%', '')) / 100.0,
                'T:大招持续治疗量': int(float(h[0].replace('%', '')) / 100.0 * health + float(h[1]) * (1 + data['属性']['治疗加成'])), }
    if data['名称'] == '九条裟罗':
        atk = float(skill_data['鸦羽天狗霆雷召咒']['数值']['攻击力加成比例'][level_e].replace('%', '')) / 100.0 * data['属性']['基础攻击']
        return {'T:乌羽攻击加成': int(atk), 'B:l0-攻击力-*': (atk,), 'B:c6-暴击伤害-*': (0.6,),
    ...
    if data['名称'] == '纳西妲':
        ...
        return {'B:l0-增伤-E': (float(skill_data['心景幻成']['数值'][f'火：伤害提升{c}'][level_q].replace('%', '')) / 100.0, f'大招计算{c}名火元素'),
                'B:l70-增伤-E': (0.8 if data['属性']['元素精通'] >= 1000 else (0.001 * (data['属性']['元素精通'] - 200)) if data['属性']['元素精通'] >= 200 else 0,),
                'B:l70-暴击率-E': (0.24 if data['属性']['元素精通'] >= 1000 else (0.0003 * (data['属性']['元素精通'] - 200)) if data['属性']['元素精通'] >= 200 else 0,),
                'B:c2-减防-*': (0.3, '二命减防触发'), 'B:l0-额外倍率-E': (float(eb[1].replace('%元素精通', '')) / 100.0 * data['属性']['元素精通'],),
                'E-e草:灭净三业': float(eb[0].replace('%攻击力', '')) / 100.0, 'E-e草-j超激化:灭净三业超激化': float(eb[0].replace('%攻击力', '')) / 100.0}
