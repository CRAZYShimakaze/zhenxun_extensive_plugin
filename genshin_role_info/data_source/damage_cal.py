...
def get_role_dmg(data: dict):
    ...
    if data["名称"] == "钟离":
        ...
        dmg_data["原岩共鸣"] = udc(
            dm["原岩共鸣"] * attack + (health * 0.019 if level_role > 60 else 0),
            (cr + ve["暴击率"], cd),
            db[6] + ve["增伤"],
            level_role,
            rcd=0.2,
        ...
        dmg_data["天星伤害"] = udc(
            dm["天星"] * attack + (health * 0.33 if level_role > 60 else 0),
            (cr + vq["暴击率"], cd),
            db[6] + vq["增伤"],
            level_role,
            rcd=0.2,
        ...
        a = udc(
            dm["踢枪"] * attack + (health * 0.0139 if level_role > 60 else 0),
            (cr + va["普攻暴击率"], cd),
            db[0] + va["普攻增伤"],
            level_role,
            rcd=0.2,
        ...
        dmg_data["梦想一刀"] = udc(
            (dm["梦想一刀基础"] + dm["梦想一刀愿力"]) * attack,
            (cr + vq["暴击率"], cd),
            db[2] + vq["增伤"] + weapon_buff,
            level_role,
            dci=dci,
        )
        dmg_data["梦想一刀超激化"] = udc(
            (dm["梦想一刀基础"] + dm["梦想一刀愿力"]) * attack + r,
            (cr + vq["暴击率"], cd),
            db[2] + vq["增伤"],
            level_role,
            dci=dci,
        ...
        a1 = udc(
            (dm["梦想一心重击基础"][0] + dm["梦想一心愿力"]) * attack,
            (cr + vq["暴击率"], cd),
            db[2] + vq["增伤"],
            level_role,
            dci=dci,
        ...
        a2 = udc(
            (dm["梦想一心重击基础"][1] + dm["梦想一心愿力"]) * attack,
            (cr + vq["暴击率"], cd),
            db[2] + vq["增伤"],
            level_role,
            dci=dci,
        ...
        extra_energy = (
            (data["属性"]["元素充能效率"] - 1) * 0.6 if level_role >= 70 else 0
        )
        dmg_data["梦想一心能量"] = (
            str(round(dm["梦想一心能量"] * (1 + extra_energy) * 5, 1)),
        )
    elif data["名称"] == "申鹤":
        ...
        if len(ab) == 1:
            ...
            dmg_data["开大普攻第一段"] = (
                str(int(ab[0]) + int(aq[0])),
                str(int(ab[1]) + int(aq[1])),
            )
        dmg_data["开大战技伤害"] = udc(
            dm["水母伤害"] * attack + dm["E伤害提升"] * health,
            (cr + ve["暴击率"], cd),
            db[3] + ve["增伤"],
            level_role,
        )
        ...
        dmg_data["开大普攻治疗量"] = (
            str(
                int(
                    (
                        float(dm["大招治疗量"][0].replace("%生命值上限", ""))
                        / 100.0
                        * health
                        + float(dm["大招治疗量"][1])
                    )
        ...
        dmg_data["战技治疗量"] = (
            str(
                int(
                    (
                        float(dm["水母治疗量"][0].replace("%生命值上限", ""))
                        / 100.0
                        * health
                        + float(dm["水母治疗量"][1])
                    )
        ...
        if skill_type == "B":
            if skill_name == "d":
                ...
                if (para[0].startswith("c") and cons >= int(para[0][1])) or (
                    para[0].startswith("l") and level_role >= int(para[0][1:])
                ):
            ...
            if len(para) != 1:
                for p in para[1:]:
                    if p.startswith("r"):
                        r = (
                            growth_reaction(data["属性"]["元素精通"], float(p[3:]))
                            if f"{p[1:3]}系数" not in data["属性"]
                            else growth_reaction(
                                data["属性"]["元素精通"],
                                float(p[3:]),
                                data["属性"][f"{p[1:3]}系数"],
                            )
            ...
            if isinstance(num, tuple):
                ...
                if n == "1":
                    dmg_data[skill_name] = udc(
                        num * dmt[t] + v[skill_type]["额外倍率"] + j,
                        (cr + v[skill_type]["暴击率"], cd + v[skill_type]["暴击伤害"]),
                        db[e] + v[skill_type]["增伤"],
                        level_role,
                        r=r,
...
def udc(
    ...
    if crit[0] > 1:
        damage = (
            ...
            * (1 + crit[1])
            * (1 + db)
            * resistance_coefficient(rcb, rcd)
            * defense_coefficient(sl, el, dcr, dci)
            * r
        )
...
def defense_coefficient(
    ...
    return (self_level + 100) / (
        (self_level + 100) + (enemy_level + 100) * (1 - reduction_rate) * (1 - ignore)
    )


def growth_reaction(
    mastery: int = 0, base_coefficient: float = 1.5, extra_coefficient: float = 0
):
    """
    计算增幅反应的系数
    :param mastery: 元素精通
...
def upheaval_reaction(
    ...
    return (
        base_coefficient
        * base_ratio
        * (1 + mastery_increase + extra_coefficient)
        * resistance
    )


def intensify_reaction(
    level: int, type: str, mastery: int = 0, extra_coefficient: float = 0
):
    """
    计算激化反应的伤害
    :param level: 等级
...
def weapon_common_fix(data: dict):
    ...
    if weapon["名称"] == "波乱月白经津":
        ...
        extra_e["额外倍率"] += (attr["基础防御"] + attr["额外防御"]) * (
            0.3 + 0.1 * weapon["精炼等级"]
        )
    elif weapon["名称"] == "腐殖之剑":
        ...
        attr["元素精通"] += (
            attr["基础生命"] * (0.0009 + 0.0003 * weapon["精炼等级"]) * 3
        )
        data["伤害描述"].append("圣显之钥满层")

    # 双手剑
    elif weapon["名称"] == "赤角石溃杵":
        extra_a["普攻额外倍率"] += (attr["基础防御"] + attr["额外防御"]) * (
            0.3 + 0.1 * weapon["精炼等级"]
        )
    ...
    # 长柄武器
        ...
        attr["额外攻击"] += (attr["基础生命"] + attr["额外生命"]) * (
            0.008 + 0.002 * weapon["精炼等级"]
        )
    ...
    # 法器
        ...
        extra_a["普攻额外倍率"] += (0.005 + 0.005 * weapon["精炼等级"]) * (
            attr["基础生命"] + attr["额外生命"]
        )
    elif weapon["名称"] == "白辰之环":
        for i, k in enumerate(attr["伤害加成"]):
    ...
    # 系列武器
        ...
        attr["额外攻击"] += (
            attr["基础攻击"] * 2 * 5 * (0.003 + 0.001 * weapon["精炼等级"])
        )
...
def get_damage_multipiler(data: dict) -> dict:
    ...
    if data["名称"] == "钟离":
        return {
            "玉璋护盾": (
                float(
                    skill_data["元素战技·地心"]["数值"]["护盾附加吸收量"][
                        level_e
                    ].replace("%最大生命值", "")
                )
                / 100.0,
                int(
                    skill_data["元素战技·地心"]["数值"]["护盾基础吸收量"][
                        level_e
                    ].replace(",", "")
                ),
            ),
            "原岩共鸣": float(
                skill_data["元素战技·地心"]["数值"]["岩脊伤害/共鸣伤害"][level_e]
                .split("/")[1]
                .replace("%", "")
            )
            / 100.0,
            "天星": float(
                skill_data["元素爆发·天星"]["数值"]["技能伤害"][level_q].replace(
                    "%", ""
                )
            )
            / 100.0,
            "踢枪": float(
                skill_data["普通攻击·岩雨"]["数值"]["五段伤害"][level_a].replace(
                    "%×4", ""
                )
            )
            / 100.0,
    ...
    if data["名称"] == "胡桃":
        ...
        dm["B:l1-攻击力"] = (
            float(
                skill_data["蝶引来生"]["数值"]["攻击力提高"][level_e].replace(
                    "%生命值上限", ""
                )
            )
            / 100.0
        ...
        dm["AZ-e火:裸重击"] = (
            float(
                skill_data["普通攻击·往生秘传枪法"]["数值"]["重击伤害"][
                    level_a
                ].replace("%", "")
            )
            / 100.0
        )
        dm["AZ-e火-r蒸发1.5:重击蒸发"] = (
            float(
                skill_data["普通攻击·往生秘传枪法"]["数值"]["重击伤害"][
                    level_a
                ].replace("%", "")
        ...
        dm["E-e火-r蒸发1.5:雪梅香蒸发"] = (
            float(
                skill_data["蝶引来生"]["数值"]["血梅香伤害"][level_e].replace("%", "")
            )
            / 100.0
        )
        dm["Q-e火-r蒸发1.5:大招蒸发"] = (
            float(
                skill_data["安神秘法"]["数值"]["低血量时技能伤害"][level_q].replace(
                    "%", ""
                )
            )
            / 100.0
    ...
    if data["名称"] == "雷电将军":
        ...
        return {
            "协同攻击": float(
                skill_data["神变·恶曜开眼"]["数值"]["协同攻击伤害"][level_e].replace(
                    "%", ""
                )
            )
            / 100.0,
            "e增伤": float(
                skill_data["神变·恶曜开眼"]["数值"]["元素爆发伤害提高"][level_e]
                .replace("每点元素能量", "")
                .replace("%", "")
            )
            / 100.0
            * 90,
            "梦想一刀基础": float(
                skill_data["奥义·梦想真说"]["数值"]["梦想一刀基础伤害"][
                    level_q
                ].replace("%", "")
            )
            / 100.0,
            "梦想一刀愿力": float(
                skill_data["奥义·梦想真说"]["数值"]["愿力加成"][level_q]
                .split("%/")[0]
                .replace("每层", "")
            )
            / 100.0
            * 60,
            "梦想一心重击基础": (
                float(qa[0].replace("%", "")) / 100.0,
                float(qa[1].replace("%", "")) / 100.0,
            ),
            "梦想一心愿力": float(
                skill_data["奥义·梦想真说"]["数值"]["愿力加成"][level_q]
                .split("%/")[1]
            ...
            * 60,
            "梦想一心能量": float(
                skill_data["奥义·梦想真说"]["数值"]["梦想一心能量恢复"][level_q]
            ),
        }
    if data["名称"] == "魈":
        a = skill_data["普通攻击·卷积微尘"]["数值"]["低空/高空坠地冲击伤害"][
            level_a
        ].split("/")
        return {
            "B:l1-增伤-AX": (
                float(
                    skill_data["靖妖傩舞"]["数值"]["普通攻击/重击/下落攻击伤害提升"][
                        level_q
                    ].replace("%", "")
                )
                / 100,
            ...
            "E-e风:风轮两立": float(
                skill_data["风轮两立"]["数值"]["技能伤害"][level_e].replace("%", "")
            )
            / 100.0,
    ...
    if data["名称"] == "香菱":
        return {
            "B:c1-减抗-*": (0.15, "锅巴减抗"),
            "E-e火:锅巴喷火": float(
                skill_data["锅巴出击"]["数值"]["喷火伤害"][level_e].replace("%", "")
            )
            / 100.0,
            "Q-e火:旋火轮": float(
                skill_data["旋火轮"]["数值"]["旋火轮伤害"][level_q].replace("%", "")
            )
            / 100.0,
    ...
    if data["名称"] == "申鹤":
        return {
            "冰翎": float(
                skill_data["仰灵威召将役咒"]["数值"]["伤害值提升"][level_e].replace(
                    "%", ""
                )
            )
            / 100.0,
            "大招减抗": float(
                skill_data["神女遣灵真诀"]["数值"]["抗性降低"][level_q].replace("%", "")
            )
            / 100.0,
            "e长按": float(
                skill_data["仰灵威召将役咒"]["数值"]["长按技能伤害"][level_e].replace(
                    "%", ""
                )
            )
            / 100.0,
            "大招持续": float(
                skill_data["神女遣灵真诀"]["数值"]["持续伤害"][level_q].replace("%", "")
            )
            / 100.0,
        }
    if data["名称"] == "刻晴":
        ...
        return {
            ...
            "AZ-e雷:重击": (
                float(az[0].replace("%", "")) / 100.0,
                float(az[1].replace("%", "")) / 100.0,
            ),
            "AZ-e雷-j超激化:重击超激化": (
                float(az[0].replace("%", "")) / 100.0,
                float(az[1].replace("%", "")) / 100.0,
            ...
            "E-e雷:战技斩击": float(
                skill_data["星斗归位"]["数值"]["斩击伤害"][level_e].replace("%", "")
            )
            / 100.0,
            ...
            / 100.0,
            "Q-e雷:大招尾刀": float(
                skill_data["天街巡游"]["数值"]["最后一击伤害"][level_q].replace("%", "")
            )
            / 100.0,
    ...
    if data["名称"] == "可莉":
        return {
            ...
            "B:c6-增伤-*": (0.1,),
            "AZ-e火:重击": float(
                skill_data["普通攻击·砰砰"]["数值"]["重击伤害"][level_a].replace(
                    "%", ""
                )
            )
            / 100.0,
            "E-e火:蹦蹦炸弹": float(
                skill_data["蹦蹦炸弹"]["数值"]["蹦蹦炸弹伤害"][level_e].replace("%", "")
            )
            / 100.0,
            "Q-e火:轰轰火花": float(
                skill_data["轰轰火花"]["数值"]["轰轰火花伤害"][level_q].replace("%", "")
            )
            / 100.0,
        }
    if data["名称"] == "八重神子":
        ...
        return {
            ...
            "B:c6-减防-E": (0.6,),
            "AZ-e雷:重击": float(
                skill_data["普通攻击·狐灵食罪式"]["数值"]["重击伤害"][level_a].replace(
                    "%", ""
                )
            )
            / 100.0,
            "E-e雷:杀生樱满阶": float(
                skill_data["野干役咒·杀生樱"]["数值"][e][level_e].replace("%", "")
            )
            / 100.0,
            ...
            / 100.0,
            "Q-e雷:天狐霆雷": float(
                skill_data["大密法·天狐显真"]["数值"]["天狐霆雷伤害"][level_q].replace(
                    "%", ""
                )
            )
            / 100.0,
    ...
    if data["名称"] == "阿贝多":
        return {
            "E-t防御力-e岩:阳华绽放": float(
                skill_data["创生法·拟造阳华"]["数值"]["刹那之花伤害"][level_e].replace(
                    "%防御力", ""
                )
            )
            / 100.0,
            "Q-e岩:大招首段": float(
                skill_data["诞生式·大地之潮"]["数值"]["爆发伤害"][level_q].replace(
                    "%", ""
                )
            )
            / 100.0,
    ...
    if data["名称"] == "神里绫华":
        return {
            ...
            "AZ-n3-e冰:重击": float(
                skill_data["普通攻击·神里流·倾"]["数值"]["重击伤害"][level_a].replace(
                    "%*3", ""
                )
            )
            / 100.0,
            "E-e冰:冰华伤害": float(
                skill_data["神里流·冰华"]["数值"]["技能伤害"][level_e].replace("%", "")
            )
            / 100.0,
            "Q-e冰:霜灭每段": float(
                skill_data["神里流·霜灭"]["数值"]["切割伤害"][level_q].replace("%", "")
            )
            / 100.0,
        }
    if data["名称"] == "行秋":
        ...
        return {
            ...
            "B:c2-减抗-*": (0.15,),
            "E-e水:画雨笼山": (
                float(e[0].replace("%", "")) / 100.0 * dm,
                float(e[1].replace("%", "")) / 100.0 * dm,
            ),
            "Q-e水:裁雨留虹每段": float(
                skill_data["古华剑·裁雨留虹"]["数值"]["剑雨伤害"][level_e].replace(
                    "%", ""
                )
            )
            / 100.0,
    ...
    if data["名称"] == "夜兰":
        return {
            ...
            "AZ-t生命值-e水:破局矢": float(
                skill_data["普通攻击·潜形隐曜弓"]["数值"]["破局矢伤害"][
                    level_a
                ].replace("%生命值上限", "")
            )
            / 100.0,
            "E-t生命值-e水:元素战技": float(
                skill_data["萦络纵命索"]["数值"]["技能伤害"][level_e].replace(
                    "%生命值上限", ""
                )
            )
            / 100.0,
            "Q-t生命值-e水-n3:大招每段": float(
                skill_data["渊图玲珑骰"]["数值"]["玄掷玲珑伤害"][level_q].replace(
                    "%生命值上限*3", ""
                )
            )
            / 100.0,
    ...
    if data["名称"] == "甘雨":
        ...
        dm["AZ-e冰:霜华矢"] = (
            float(
                skill_data["普通攻击·流天射术"]["数值"]["霜华矢命中伤害"][
                    level_a
                ].replace("%", "")
            )
            / 100.0,
            float(
                skill_data["普通攻击·流天射术"]["数值"]["霜华矢·霜华绽发伤害"][
                    level_a
                ].replace("%", "")
            )
            / 100.0,
        ...
        dm["E-e冰:元素战技"] = (
            float(skill_data["山泽麟迹"]["数值"]["技能伤害"][level_e].replace("%", ""))
            / 100.0
        )
        dm["Q-e冰:冰棱伤害"] = (
            float(skill_data["降众天华"]["数值"]["冰棱伤害"][level_q].replace("%", ""))
            / 100.0
        )
        return dm
    if data["名称"] == "优菈":
        ...
        return {
            ...
            "B:l1-减抗-*": (
                float(
                    skill_data["冰潮的涡旋"]["数值"]["物理抗性降低"][level_e].replace(
                        "%", ""
                    )
                )
                / 100.0,
            ...
            "A:普攻第一段": float(
                skill_data["普通攻击·西风剑术·宗室"]["数值"]["一段伤害"][
                    level_a
                ].replace("%", "")
            )
            / 100.0,
            "E-e冰:战技长按": float(
                skill_data["冰潮的涡旋"]["数值"]["长按伤害"][level_e].replace("%", "")
            )
            / 100.0,
            f"Q:光降之剑{n}层": float(
                skill_data["凝浪之光剑"]["数值"]["光降之剑基础伤害"][level_q].replace(
                    "%", ""
                )
            )
            / 100.0
            + float(
                skill_data["凝浪之光剑"]["数值"]["每层能量伤害"][level_e].replace(
                    "%", ""
                )
            )
            / 100.0
    ...
    if data["名称"] == "达达利亚":
        ...
        return {
            "E-e水:近战重击": (
                float(e[0].replace("%", "")) / 100.0,
                float(e[1].replace("%", "")) / 100.0,
            ),
            "E-e水:断流·斩": float(
                skill_data["魔王武装·狂澜"]["数值"]["断流·斩伤害"][level_e].replace(
                    "%", ""
                )
            )
            / 100.0,
            "A-e水:断流·破": float(
                skill_data["普通攻击·断雨"]["数值"]["断流·破 伤害"][level_a].replace(
                    "%", ""
                )
            )
            / 100.0,
            "Q-e水:近战大招": float(
                skill_data["极恶技·尽灭闪"]["数值"]["技能伤害·近战"][level_q].replace(
                    "%", ""
                )
            )
            / 100.0,
            ...
            / 100.0,
            "Q-e水:断流·爆": float(
                skill_data["极恶技·尽灭闪"]["数值"]["断流·爆伤害"][level_q].replace(
                    "%", ""
                )
            )
            / 100.0,
    ...
    if data["名称"] == "迪卢克":
        return {
            ...
            "B:c1-增伤-*": (0.15, "一命增伤"),
            "E-e火:战技第三段": float(
                skill_data["逆焰之刃"]["数值"]["三段伤害"][level_e].replace("%", "")
            )
            / 100.0,
            ...
            / 100.0,
            "Q-e火:大招斩击": float(
                skill_data["黎明"]["数值"]["斩击伤害"][level_q].replace("%", "")
            )
            / 100.0,
    ...
    if data["名称"] == "凝光":
        return {
            "B:l70-增伤-*": (0.12, "穿屏增伤"),
            "AZ-e岩:重击": float(
                skill_data["普通攻击·千金掷"]["数值"]["重击伤害"][level_a].replace(
                    "%", ""
                )
            )
            / 100.0,
            "AZ-e岩:星璇伤害": float(
                skill_data["普通攻击·千金掷"]["数值"]["星璇伤害"][level_a]
                .replace("%", "")
                .replace("每个", "")
            )
            / 100.0,
            "E-e岩:璇玑屏": float(
                skill_data["璇玑屏"]["数值"]["技能伤害"][level_e].replace("%", "")
            )
            / 100.0,
            "Q-e岩:大招每段": float(
                skill_data["天权崩玉"]["数值"]["每颗宝石伤害"][level_q]
                .replace("%", "")
                .replace("每个", "")
            )
            / 100.0,
    ...
    if data["名称"] == "菲谢尔":
        dm = {
            "A:普攻第一段": float(
                skill_data["普通攻击·罪灭之矢"]["数值"]["一段伤害"][level_a].replace(
                    "%", ""
                )
            )
            / 100.0,
            "E-e雷:奥兹攻击": float(
                skill_data["夜巡影翼"]["数值"]["奥兹攻击伤害"][level_e].replace("%", "")
            )
            / 100.0,
    ...
    if data["名称"] == "北斗":
        return {
            "B:c6-减抗-*": (0.15, "六命减抗"),
            "E-e雷:完美弹反": float(
                skill_data["捉浪"]["数值"]["基础伤害"][level_e].replace("%", "")
            )
            / 100.0
            + 2
            * float(
                skill_data["捉浪"]["数值"]["每层伤害提升"][level_e].replace("%", "")
            )
            / 100.0,
            ...
            * float(
            ...
            / 100.0,
            "Q-e雷:斫雷每段": float(
                skill_data["斫雷"]["数值"]["闪电伤害"][level_q].replace("%", "")
            )
            / 100.0,
        }
    if data["名称"] == "诺艾尔":
        ...
        return {
            "B:l1-攻击力": (
                (
                    float(
                        skill_data["大扫除"]["数值"]["攻击力提高"][level_q].replace(
                            "%防御力", ""
                        )
                    )
                    / 100.0
            ...
            "A-e岩:普攻第一段": float(
                skill_data["普通攻击·西风剑术·女仆"]["数值"]["一段伤害"][
                    level_a
                ].replace("%", "")
            )
            / 100.0,
            ...
            "T:E盾值": int(
                float(e[0].replace("%防御力", "")) / 100.0 * defense + float(e[1]) * 1.5
            ),
            "T:普攻治疗量/概率": str(
                int(
                    (
                        float(ez[0].replace("%防御力", "")) / 100.0 * defense
                        + float(ez[1])
                    )
                    * (1 + data["属性"]["治疗加成"])
                )
    ...
    if data["名称"] == "神里绫人":
        ...
        return {
            ...
            "A-e水:瞬水剑第一段": float(
                skill_data["神里流·镜花"]["数值"]["一段瞬水剑伤害"][level_e].replace(
                    "%", ""
                )
            )
            / 100.0,
            "Q-e水:大招每下": float(
                skill_data["神里流·水囿"]["数值"]["水花剑伤害"][level_q].replace(
                    "%", ""
                )
            )
            / 100.0,
    ...
    if data["名称"] == "荒泷一斗":
        return {
            "B:d": ["开启大招"],
            "B:l70-额外倍率-AZ": (
                0.35 * (data["属性"]["基础防御"] + data["属性"]["额外防御"]),
            ),
            "B:c6-暴击伤害-AZ": (0.7,),
            "B:l1-攻击力": (
                float(
                    skill_data["最恶鬼王·一斗轰临！！"]["数值"]["攻击力提高"][
                        level_q
                    ].replace("%防御力", "")
                )
                / 100.0
            ...
            "AZ-e岩:荒泷逆袈裟连斩": float(
                skill_data["普通攻击·喧哗屋传说"]["数值"]["荒泷逆袈裟连斩伤害"][
                    level_a
                ].replace("%", "")
            )
            / 100.0,
            "E-e岩:赤牛发破": float(
                skill_data["魔杀绝技·赤牛发破！"]["数值"]["技能伤害"][level_e].replace(
                    "%", ""
                )
            )
            / 100.0,
    ...
    if data["名称"] == "宵宫":
        return {
            ...
            "B:c2-增伤-*": (0.25, "二命触发"),
            "A-e火-n2:普攻第一段": float(
                skill_data["焰硝庭火舞"]["数值"]["炽焰箭伤害"][level_e].replace(
                    "%普通攻击伤害", ""
                )
            )
            / 100.0
            * float(
                skill_data["普通攻击·烟火打扬"]["数值"]["一段伤害"][level_a].replace(
                    "%*2", ""
                )
            )
            / 100.0,
            ...
            * float(
                skill_data["普通攻击·烟火打扬"]["数值"]["三段伤害"][level_a].replace(
                    "%", ""
                )
            )
            / 100.0,
            "Q-e火:琉金火光爆炸": float(
                skill_data["琉金云间草"]["数值"]["琉金火光爆炸伤害"][level_q].replace(
                    "%", ""
                )
            )
            / 100.0,
    ...
    if data["名称"] == "烟绯":
        ...
        AZB = (
            float(
                skill_data["普通攻击·火漆制印"]["数值"]["重击伤害"][level_a].replace(
                    "%", ""
                )
            )
            / 100.0
        ...
        return {
            ...
            "B:l1-增伤-AZ": (
                float(
                    skill_data["凭此结契"]["数值"]["重击伤害提升"][level_q].replace(
                        "%", ""
                    )
                )
                / 100.0,
            ...
            "AZ-e火:满丹火印重击": AZ,
            "E-e火:元素战技": float(
                skill_data["丹书立约"]["数值"]["技能伤害"][level_e].replace("%", "")
            )
            / 100.0,
            "Q-e火:元素爆发": float(
                skill_data["凭此结契"]["数值"]["技能伤害"][level_q].replace("%", "")
            )
            / 100.0,
        }
    if data["名称"] == "珊瑚宫心海":
        return {
            "普攻第一段": float(
                skill_data["普通攻击·水有常形"]["数值"]["一段伤害"][level_a].replace(
                    "%", ""
                )
            )
            / 100.0,
            "水母伤害": float(
                skill_data["海月之誓"]["数值"]["波纹伤害"][level_e].replace("%", "")
            )
            / 100.0,
            "水母治疗量": skill_data["海月之誓"]["数值"]["治疗量"][level_e].split("+"),
            "大招伤害": float(
                skill_data["海人化羽"]["数值"]["技能伤害"][level_q].replace(
                    "%生命值上限", ""
                )
            )
            / 100.0,
            "普攻伤害提升": float(
                skill_data["海人化羽"]["数值"]["普通攻击伤害提升"][level_q].replace(
                    "%生命值上限", ""
                )
            )
            / 100.0,
            "E伤害提升": float(
                skill_data["海人化羽"]["数值"]["化海月伤害提升"][level_q].replace(
                    "%生命值上限", ""
                )
            )
            / 100.0,
    ...
    if data["名称"] == "枫原万叶":
        ...
        return {
            ...
            "AX-e风:E后高空下落": float(
                skill_data["普通攻击·我流剑术"]["数值"]["低空/高空坠地冲击伤害"][
                    level_a
                ]
                .split("/")[1]
                .replace("%", "")
            ...
            / 100.0,
            "E-e风:E点按伤害": float(
                skill_data["千早振"]["数值"]["点按技能伤害"][level_e].replace("%", "")
            )
            / 100.0,
            "Q-e风:大招斩击": float(
                skill_data["万叶之一刀"]["数值"]["斩击伤害"][level_q].replace("%", "")
            )
            / 100.0,
            "Q-e风:大招持续": float(
                skill_data["万叶之一刀"]["数值"]["持续伤害"][level_q].replace("%", "")
            )
            / 100.0,
            "T:扩散伤害": int(
                upheaval_reaction(data["等级"], "扩散", data["属性"]["元素精通"], up)
            ),
        }
    if data["名称"] == "鹿野院平藏":
        e = skill_data["勠心拳"]["数值"]
        ...
        return {
            ...
            "B:c6-暴击伤害-E": (0.32,),
            "AZ-e风:重击": float(
                skill_data["普通攻击·不动流格斗术"]["数值"]["重击伤害"][
                    level_a
                ].replace("%", "")
            )
            / 100.0,
            "E-e风:满层勠心拳": (
                float(e["技能伤害"][level_e].replace("%", ""))
                + 4 * float(e["变格伤害提升"][level_e].replace("%/层", ""))
                + float(e["正论伤害提升"][level_e].replace("%", ""))
            )
            / 100.0,
            "Q-e风:聚风蹴真空弹": float(
                skill_data["聚风蹴"]["数值"]["不动流·真空弹伤害"][level_q].replace(
                    "%", ""
                )
            )
            / 100.0,
    ...
    if data["名称"] == "班尼特":
        ...
        return {
            ...
            "E-e火:元素战技点按": float(
                skill_data["热情过载"]["数值"]["点按伤害"][level_e].replace("%", "")
            )
            / 100.0,
            "Q-e火:大招伤害": float(
                skill_data["美妙旅程"]["数值"]["技能伤害"][level_q].replace("%", "")
            )
            / 100.0,
            "T:大招攻击加成": int(attack_increase * data["属性"]["基础攻击"]),
            "T:大招持续治疗": int(
                (
                    float(hp_recover[0].replace("每秒", "").replace("%生命值上限", ""))
                    / 100.0
                    * health
                    + int(hp_recover[1])
                )
    ...
    if data["名称"] == "温迪":
        ...
        return {
            ...
            "B:c6-减抗-*": (0.2, "六命减抗"),
            "E-e风:E点按伤害": float(
                skill_data["高天之歌"]["数值"]["点按伤害"][level_e].replace("%", "")
            )
            / 100.0,
            "Q-e风:大招持续伤害": float(
                skill_data["风神之诗"]["数值"]["持续伤害"][level_q].replace("%", "")
            )
            / 100.0,
            "T:扩散伤害": int(
                upheaval_reaction(data["等级"], "扩散", data["属性"]["元素精通"], up)
    ...
    if data["名称"] == "莫娜":
        ...
        return {
            ...
            "B:l1-增伤-*": (
                float(
                    skill_data["星命定轨"]["数值"]["伤害加成"][level_q].replace("%", "")
                )
                / 100.0,
                "星异增伤",
            ),
            "AZ-e水:重击": float(
                skill_data["普通攻击·因果点破"]["数值"]["重击伤害"][level_a].replace(
                    "%", ""
                )
            )
            / 100.0,
            "E-e水:E持续伤害": float(
                skill_data["水中幻愿"]["数值"]["持续伤害"][level_e].replace("%", "")
            )
            / 100.0,
            "Q-e水:泡影破裂": float(
                skill_data["星命定轨"]["数值"]["泡影破裂伤害"][level_q].replace("%", "")
            )
            / 100.0,
    ...
    if data["名称"] == "琴":
        ...
        return {
            ...
            "B:c4-减抗-Q": (0.4,),
            "AZ:重击": float(
                skill_data["普通攻击·西风剑术"]["数值"]["重击伤害"][level_a].replace(
                    "%", ""
                )
            )
            / 100.0,
            "E-e风:风压剑": float(
                skill_data["风压剑"]["数值"]["技能伤害"][level_e].replace("%", "")
            )
            / 100.0,
            "Q-e风:大招爆发伤害": float(
                skill_data["蒲公英之风"]["数值"]["爆发伤害"][level_q].replace("%", "")
            )
            / 100.0,
            "Q-e风:大招出入领域伤害": float(
                skill_data["蒲公英之风"]["数值"]["出入领域伤害"][level_q].replace(
                    "%", ""
                )
            )
            / 100.0,
            "T:大招瞬时治疗": int(
                (
                    float(recovery1[0].replace("%攻击力", "")) / 100.0 * attack
                    + int(recovery1[1])
                )
                * (1 + data["属性"]["治疗加成"])
            ),
            "T:大招持续治疗": int(
                (
                    float(recovery2[0].replace("每秒", "").replace("%攻击力", ""))
                    / 100.0
                    * attack
                    + int(recovery2[1])
                )
    ...
    if data["名称"] == "七七":
        ...
        return {
            "B:c1-增伤-AZ": (0.15,),
            "AZ:重击": (
                float(a[0].replace("%", "")) / 100.0,
                float(a[0].replace("%", "")) / 100.0,
            ),
            "E-e冰:E释放伤害": float(
                skill_data["仙法·寒病鬼差"]["数值"]["技能伤害"][level_e].replace(
                    "%", ""
                )
            )
            / 100.0,
            "T:开E后普攻治疗量": int(
                (
                    float(a_rec[0].replace("%攻击力", "")) / 100.0 * attack
                    + int(a_rec[1])
                )
                * (1 + data["属性"]["治疗加成"])
            ),
            "T:E持续治疗量": int(
                (
                    float(e_rec[0].replace("%攻击力", "")) / 100.0 * attack
                    + int(e_rec[1])
                )
                * (1 + data["属性"]["治疗加成"])
            ),
            "T:挂符每次治疗量": int(
                (
                    float(q_rec[0].replace("%攻击力", "")) / 100.0 * attack
                    + int(q_rec[1])
                )
                * (1 + data["属性"]["治疗加成"])
            ),
            "Q-e冰:大招伤害": float(
                skill_data["仙法·救苦度厄"]["数值"]["技能伤害"][level_q].replace(
                    "%", ""
                )
            )
            / 100.0,
    ...
    if data["名称"] == "提纳里":
        ...
        return {
            ...
            "AZ-e草:重击花筥箭": float(
                skill_data["普通攻击·藏蕴破障"]["数值"]["花筥箭伤害"][level_a].replace(
                    "%", ""
                )
            )
            / 100.0,
            ...
            "AZ-e草-n4:重击藏蕴花矢": float(
                skill_data["普通攻击·藏蕴破障"]["数值"]["藏蕴花矢伤害"][
                    level_a
                ].replace("%", "")
            )
            / 100.0,
            "Q-e草:缠藤箭": float(
                skill_data["造生缠藤箭"]["数值"]["缠藤箭伤害"][level_q].replace("%", "")
            )
            / 100.0,
            ...
            / 100.0,
            "Q-e草:次级缠藤箭-n6": float(
                skill_data["造生缠藤箭"]["数值"]["次级缠藤箭伤害"][level_q].replace(
                    "%", ""
                )
            )
            / 100.0,
    ...
    if data["名称"] == "柯莱":
        ...
        return {
            "E-e草:飞叶轮": float(
                skill_data["拂花偈叶"]["数值"]["技能伤害"][level_e].replace("%", "")
            )
            / 100.0,
            ...
            / 100.0,
            "Q-e草:大招爆发伤害": float(
                skill_data["猫猫秘宝"]["数值"]["爆发伤害"][level_q].replace("%", "")
            )
            / 100.0,
            ...
            / 100.0,
            "Q-e草:大招持续伤害": float(
                skill_data["猫猫秘宝"]["数值"]["跃动伤害"][level_q].replace("%", "")
            )
            / 100.0,
    ...
    if data["名称"] == "多莉":
        h = skill_data["卡萨扎莱宫的无微不至"]["数值"]["持续治疗量"][level_q].split(
            "生命值上限+"
        )
        return {
            "E-e雷:断除烦恼炮": float(
                skill_data["镇灵之灯·烦恼解决炮"]["数值"]["断除烦恼炮伤害"][
                    level_e
                ].replace("%", "")
            )
            / 100.0,
            "T:大招持续治疗量": int(
                float(h[0].replace("%", "")) / 100.0 * health
                + float(h[1]) * (1 + data["属性"]["治疗加成"])
            ),
        }
    if data["名称"] == "九条裟罗":
        atk = (
            float(
                skill_data["鸦羽天狗霆雷召咒"]["数值"]["攻击力加成比例"][
                    level_e
                ].replace("%", "")
            )
            / 100.0
        ...
        return {
            ...
            "B:c6-暴击伤害-*": (0.6,),
            "E-e雷:元素战技": float(
                skill_data["鸦羽天狗霆雷召咒"]["数值"]["天狗咒雷•伏伤害"][
                    level_e
                ].replace("%", "")
            )
            / 100.0,
            "Q-e雷:大招首段": float(
                skill_data["煌煌千道镇式"]["数值"]["天狗咒雷•金刚坏伤害"][
                    level_q
                ].replace("%", "")
            )
            / 100.0,
            ...
            / 100.0,
            "Q-e雷:大招雷砾": float(
                skill_data["煌煌千道镇式"]["数值"]["天狗咒雷•雷砾伤害"][
                    level_q
                ].replace("%", "")
            )
            / 100.0,
    ...
    if data["名称"] == "纳西妲":
        ...
        return {
            "B:l0-增伤-E": (
                float(
                    skill_data["心景幻成"]["数值"][f"火：伤害提升{c}"][level_q].replace(
                        "%", ""
                    )
                )
                / 100.0,
            ...
            "B:l70-增伤-E": (
                0.8
                if data["属性"]["元素精通"] >= 1000
                else (0.001 * (data["属性"]["元素精通"] - 200))
                if data["属性"]["元素精通"] >= 200
                else 0,
            ),
            "B:l70-暴击率-E": (
                0.24
                if data["属性"]["元素精通"] >= 1000
                else (0.0003 * (data["属性"]["元素精通"] - 200))
                if data["属性"]["元素精通"] >= 200
                else 0,
            ),
            "B:c2-减防-*": (0.3, "二命减防触发"),
            "B:l0-额外倍率-E": (
                float(eb[1].replace("%元素精通", ""))
                / 100.0
                * data["属性"]["元素精通"],
            ),
