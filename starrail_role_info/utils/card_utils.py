...
class PlayerInfo:
    ...
    def set_role(self, data: dict):
        ...
        prop = {}
        prop["基础生命值"] = self.get_role_value(
            "hp", role_info["角色ID"], role_info["晋升"], role_info["等级"]
        )
        prop["基础攻击力"] = self.get_role_value(
            "atk", role_info["角色ID"], role_info["晋升"], role_info["等级"]
        )
        prop["基础防御力"] = self.get_role_value(
            "def", role_info["角色ID"], role_info["晋升"], role_info["等级"]
        )
        prop["基础暴击率"] = self.get_role_value(
            "crit_rate", role_info["角色ID"], role_info["晋升"], role_info["等级"]
        )
        prop["基础暴击伤害"] = self.get_role_value(
            "crit_dmg", role_info["角色ID"], role_info["晋升"], role_info["等级"]
        )
        prop["基础速度"] = self.get_role_value(
            "spd", role_info["角色ID"], role_info["晋升"], role_info["等级"]
        )
        ...
        if data.get("equipment", ""):
            ...
            weapon_info["生命值"] = self.get_weapon_value(
                "hp", weapon_data["tid"], weapon_info["突破等级"], weapon_info["等级"]
            )
            weapon_info["攻击力"] = self.get_weapon_value(
                "atk", weapon_data["tid"], weapon_info["突破等级"], weapon_info["等级"]
            )
            weapon_info["防御力"] = self.get_weapon_value(
                "def", weapon_data["tid"], weapon_info["突破等级"], weapon_info["等级"]
            )

            if weapon_prop[weapon_data["tid"]]["properties"][
                weapon_info["精炼等级"] - 1
            ]:
                weapon_prop_info = weapon_prop[weapon_data["tid"]]["properties"][
                    weapon_info["精炼等级"] - 1
                ]
        ...
        for item in data.get("skillTreeList"):
            if skill_prop := role_skill.get(str(item["pointId"]))["levels"][0][
                "properties"
            ]:
                skill_prop = {
            ...
            for relic_info in relic_list:
                ...
                artifact_info["主属性"] = {
                    "属性名": trans_data["property"][
                        relic_main_value[relic_item["main_affix_id"]]["affixes"][
                            str(relic_info["mainAffixId"])
                        ]["property"]
                    ],
                ...
                for sub_info in relic_info.get("subAffixList", []):
                    artifact_info["词条"].append(
                        {
                            "属性名": trans_data["property"][
                                relic_sub_value[relic_item["sub_affix_id"]]["affixes"][
                                    str(sub_info["affixId"])
                                ]["property"]
                            ],
    ...
    def cal_prop(self, prop, main, sub):
        if "百分比" in main["属性名"]:
            prop[f"额外{main['属性名'].replace('百分比', '')}"] += (
                prop[f"基础{main['属性名'].replace('百分比', '')}"] * main["属性值"]
            )
        ...
        for item in sub:
            if "百分比" in item["属性名"]:
                prop[f"额外{item['属性名'].replace('百分比', '')}"] += (
                    prop[f"基础{item['属性名'].replace('百分比', '')}"] * item["属性值"]
                )
    ...
    def get_role_value(self, item, id, prop, level):
        if prop == 0:
            val = role_value[id]["values"][prop][item]["base"] + role_value[id][
                "values"
            ][prop][item]["step"] * (level - 1)
        else:
            val = role_value[id]["values"][prop][item]["base"] + role_value[id][
                "values"
            ][prop][item]["step"] * (level - 1)
    ...
    def get_weapon_value(self, item, id, prop, level):
        if prop == 0:
            val = weapon_val[id]["values"][prop][item]["base"] + weapon_val[id][
                "values"
            ][prop][item]["step"] * (level - 1)
        else:
            val = weapon_val[id]["values"][prop][item]["base"] + weapon_val[id][
    ...
    def get_main_relic_value(self, id, affixes_id, level):
        val = (
            relic_main_value[id]["affixes"][str(affixes_id)]["base"]
            + relic_main_value[id]["affixes"][str(affixes_id)]["step"] * level
        )  # ((level if level else 1) - 1)
        return val

    def get_sub_relic_value(self, id, affixes_id, level, step_num):
        val = (
            relic_sub_value[id]["affixes"][str(affixes_id)]["base"] * level
