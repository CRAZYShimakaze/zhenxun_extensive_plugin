...
class PlayerInfo:
    ...
    def set_role(self, data: dict):
        ...
        for i in range(data.get("rank", 0)):
            talent_detail = {"图标": f"{role_info['角色ID']}_skill" if i == 2 else f"{role_info['角色ID']}_technique" if i == 4 else f"{role_info['角色ID']}_rank{i + 1}"}
        ...
        if data.get("equipment", ""):
            ...
            if weapon_prop[weapon_data["tid"]]["properties"][weapon_info["精炼等级"] - 1]:
                ...
                if weapon_prop_info[0]["type"] != "AllDamageTypeAddedRatio":
                    weapon_prop_info = {"属性名": trans_data["property"][weapon_prop_info[0]["type"]], "属性值": weapon_prop_info[0]["value"]}
                else:
                    weapon_prop_info = {"属性名": "能量恢复效率", "属性值": 0}
        ...
        role_info["行迹"] = [
            {"名称": "普攻", "等级": data.get("skillTreeList")[0]["level"], "图标": f"{role_info['角色ID']}_basic_atk"},
            {"名称": "战技", "等级": data.get("skillTreeList")[1]["level"], "图标": f"{role_info['角色ID']}_skill"},
            {"名称": "终结技", "等级": data.get("skillTreeList")[2]["level"], "图标": f"{role_info['角色ID']}_ultimate"},
            {"名称": "天赋", "等级": data.get("skillTreeList")[3]["level"], "图标": f"{role_info['角色ID']}_talent"},
        ]
        ...
        for item in data.get("skillTreeList"):
            if skill_prop := role_skill.get(str(item["pointId"]))["levels"][0]["properties"]:
                skill_prop = {"属性名": trans_data["property"][skill_prop[0]["type"]], "属性值": skill_prop[0]["value"]}
            ...
            for suit in suit_4:
                ...
                for item in relic_prop:
                    if item:
                        for i in item:
                            if i:
                                relic_suit_prop.append({"属性名": trans_data["property"][i["type"]], "属性值": i["value"]})
        if suit_2:
            for suit in suit_2:
                relic_prop = relic_sets[suit]["properties"]
                for item in relic_prop:
                    if item:
                        relic_suit_prop.append({"属性名": trans_data["property"][item[0]["type"]], "属性值": item[0]["value"]})
    ...
    def get_main_relic_value(self, id, affixes_id, level):
        val = relic_main_value[id]["affixes"][str(affixes_id)]["base"] + relic_main_value[id]["affixes"][str(affixes_id)]["step"] * level  # ((level if level else 1) - 1)
        return val
