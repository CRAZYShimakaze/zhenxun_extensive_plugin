...
from ..utils.artifact_utils import get_effective, check_effective, \
    get_artifact_score
from ..utils.card_utils import json_path, other_path, get_font, bg_path, char_pic_path, skill_path, path_path, \
    outline_path, talent_path, weapon_path, reli_path, trans_data
...
async def draw_role_card(uid, data, player_info, plugin_version, only_cal):
    ...
    if not only_cal:
        bg_card = load_image(f'{bg_path}/背景_{data["元素"]}.png', size=(1080, 1920),
                             mode='RGBA')
        try:
        ...
        element_icon = await get_img(
            url=element_url.format(data["元素"]),
            save_path=element_icon,
            mode='RGBA')
        ...
        draw_center_text(bg_draw, f'LV{data["等级"]}',
                         ...
                         298 + 60 * (len(data['名称']) - 2) + 171, 114, 'black',
                         get_font(48, 'number.ttf'))
        # 属性值
        prop = data['属性']
        bg_draw.text((89, 202), '生命值', fill='white', font=get_font(34, 'hywh.ttf'))
        text_length = bg_draw.textlength(f"+{int(prop['额外生命值'])}",
                                         font=get_font(34, 'number.ttf'))
        draw_right_text(bg_draw, f"{int(prop['基础生命值'])}", 480 - text_length - 5, 204,
                        'white', get_font(34, 'number.ttf'))
        draw_right_text(bg_draw, f"+{int(prop['额外生命值'])}", 480, 204, '#59c538',
                        get_font(34, 'number.ttf'))

        bg_draw.text((89, 259), '攻击力', fill='white', font=get_font(34, 'hywh.ttf'))
        text_length = bg_draw.textlength(f"+{int(prop['额外攻击力'])}",
                                         font=get_font(34, 'number.ttf'))
        draw_right_text(bg_draw, f"{int(prop['基础攻击力'])}", 480 - text_length - 5, 261,
                        'white', get_font(34, 'number.ttf'))
        draw_right_text(bg_draw, f"+{int(prop['额外攻击力'])}", 480, 261, '#59c538',
                        get_font(34, 'number.ttf'))

        bg_draw.text((89, 317), '防御力', fill='white', font=get_font(34, 'hywh.ttf'))
        text_length = bg_draw.textlength(f"+{int(prop['额外防御力'])}",
                                         font=get_font(34, 'number.ttf'))
        draw_right_text(bg_draw, f"{int(prop['基础防御力'])}", 480 - text_length - 5, 319,
                        'white', get_font(34, 'number.ttf'))
        draw_right_text(bg_draw, f"+{int(prop['额外防御力'])}", 480, 319, '#59c538',
                        get_font(34, 'number.ttf'))

        bg_draw.text((89, 377), '速度', fill='white', font=get_font(34, 'hywh.ttf'))
        text_length = bg_draw.textlength(f"+{int(prop['额外速度'])}",
                                         font=get_font(34, 'number.ttf'))
        draw_right_text(bg_draw, f"{int(prop['基础速度'])}", 480 - text_length - 5, 379,
                        'white', get_font(34, 'number.ttf'))
        draw_right_text(bg_draw, f"+{int(prop['额外速度'])}", 480, 379, '#59c538',
                        get_font(34, 'number.ttf'))
        ...
        bg_draw.text((89, 551),
                     ...
                     fill='white',
                     font=get_font(34, 'hywh.ttf'))
        draw_right_text(bg_draw, f"{text}%", 480, 553, 'white',
                        get_font(34, 'number.ttf'))
        ...
        for i in range(4):
            ...
            draw_center_text(bg_draw, str(data['行迹'][i]['等级']), 525 + 117 * i,
                             567 + 117 * i, 310 + 470, 'white',
                             get_font(34, 'number.ttf'))
        ...
        draw_center_text(bg_draw, f'LV{data["光锥"]["等级"]}', 268, 268 + 98, 835,
    ...
    for i in range(6):
        ...
        player_info.data['大毕业遗器'] = player_info.data['大毕业遗器'] + 1 if artifact_score == 'ACE' else \
            player_info.data['大毕业遗器']
        player_info.data['小毕业遗器'] = player_info.data['小毕业遗器'] + 1 if artifact_score == 'SSS' else \
        ...
        bg_draw.text((94 + 317 * offset_x, 951 + offset_y),
                     artifact['名称'],
                     fill='white',
                     font=get_font(30))
        bg_draw.text((95 + 317 * offset_x, 998 + offset_y),
        ...
        if artifact['等级'] != 15:
            no_list = '*'
        draw_center_text(bg_draw, f"LV{artifact['等级']}", 95 + 317 * offset_x,
                         95 + 317 * offset_x + 98, 1033 + offset_y, 'black',
                         get_font(27, 'number.ttf'))
        bg_draw.text((94 + 317 * offset_x, 1069 + offset_y),
                     artifact['主属性']['属性名'],
                     fill='white',
                     font=get_font(25))
        if artifact['主属性']['属性名'] not in integer_property:
            bg_draw.text((91 + 317 * offset_x, 1100 + offset_y),
        ...
        for j in range(len(artifact['词条'])):
            ...
            artifact_pk_info['副属性'].append({'属性名': text, '属性值': artifact['词条'][j]['属性值'], '强化次数': up_num,
    ...
    player_info.data['遗器榜单'] = sorted(player_info.data['遗器榜单'], key=lambda x: float(x['评分']),
                                          reverse=True)[:20]
    if not only_cal:
        # 遗器评分
        if total_cnt and total_all <= 55 * total_cnt:
            ...
            '''
            total_rank = 'ACE' if total_all > 262.0 \
                else 'SSS' if total_all > 227.0 else 'SS' if total_all > 200.8 else 'S' if total_all > 174.6 else 'A' \
                if total_all > 139.7 else 'B' if total_all > 104.8 else 'C' if total_all > 69.9 else 'D'
        else:
            total_rank = 'D'
        ...
        if total_rank == 'ACE':
            ...
            bg_draw.text((250 + score_x_offset, 974 + score_y_offset),
                         str(total_int),
                         fill='white',
            ...
            bg_draw.text((235 + score_x_offset, 974 + score_y_offset),
                         str(total_int),
                         fill='white',
                         font=get_font(60, 'number.ttf'))
        else:
            bg.alpha_composite(rank_icon, (143 + x_offset - 18, 967 + y_offset))
            bg_draw.text((217 + score_x_offset, 974 + score_y_offset),
                         str(total_int),
                         fill='white',
                         font=get_font(60, 'number.ttf'))
        '''
    ...
    for item in effective:
        if item == '伤害加成':
            ...
            name = item.replace('百分比', '').replace('效率', '').replace('暴击率', '暴击').replace(
                '暴击伤害', '爆伤').replace('治疗加成', '治疗')
    ...
    draw_center_text(bg_draw, f'Updated on {date[:-3]} | v{plugin_version} | Powered by miHoYo',
