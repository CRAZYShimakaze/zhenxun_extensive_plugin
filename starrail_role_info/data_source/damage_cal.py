...
def get_dmg_data(data, dm, va, ve, vq):
    ...
    v = {
        "A": {"暴击率": va["普攻暴击率"], "暴击伤害": 0, "增伤": va["普攻增伤"], "额外倍率": va["普攻额外倍率"], "减抗": va["减抗"], "减防": 0},
    ...
    dmt = {"攻击力": data["属性"]["基础攻击"] + data["属性"]["额外攻击"], "生命值": data["属性"]["基础生命"] + data["属性"]["额外生命"], "防御力": data["属性"]["基础防御"] + data["属性"]["额外防御"]}
    cr = data["属性"]["暴击率"]
    cd = data["属性"]["暴击伤害"]
    db = data["属性"]["伤害加成"]
    cons = len(data["命座"])
    ...
    for name, num in dm.items():
        ...
        if skill_type == "B":
            ...
            if len(para) != 1:
                for p in para[1:]:
                    if p.startswith("r"):
                        r = growth_reaction(data["属性"]["元素精通"], float(p[3:])) if f"{p[1:3]}系数" not in data["属性"] else growth_reaction(data["属性"]["元素精通"], float(p[3:]), data["属性"][f"{p[1:3]}系数"])
                    if p.startswith("j"):
...
def weapon_common_fix(data: dict):
    ...
    # 针对a的额外属性
    extra_a = {"普攻暴击率": 0, "普攻增伤": 0, "普攻额外倍率": 0, "重击暴击率": 0, "重击增伤": 0, "重击额外倍率": 0, "下落攻击暴击率": 0, "下落攻击增伤": 0, "下落攻击额外倍率": 0, "减抗": 0}
    # 单手剑
    if weapon["名称"] == "波乱月白经津":
...
def get_damage_multipiler(data: dict) -> dict:
    ...
    if data["名称"] == "钟离":
        return {
            "玉璋护盾": (float(skill_data["元素战技·地心"]["数值"]["护盾附加吸收量"][level_e].replace("%最大生命值", "")) / 100.0, int(skill_data["元素战技·地心"]["数值"]["护盾基础吸收量"][level_e].replace(",", ""))),
    ...
    if data["名称"] == "优菈":
        ...
        return {
            ...
            f"Q:光降之剑{n}层": float(skill_data["凝浪之光剑"]["数值"]["光降之剑基础伤害"][level_q].replace("%", "")) / 100.0
            + float(skill_data["凝浪之光剑"]["数值"]["每层能量伤害"][level_e].replace("%", "")) / 100.0 * n,
        }
    if data["名称"] == "达达利亚":
    ...
    if data["名称"] == "菲谢尔":
        dm = {
            "A:普攻第一段": float(skill_data["普通攻击·罪灭之矢"]["数值"]["一段伤害"][level_a].replace("%", "")) / 100.0,
    ...
    if data["名称"] == "北斗":
        return {
            ...
            "E-e雷-j超激化:完美弹反超激化": float(skill_data["捉浪"]["数值"]["基础伤害"][level_e].replace("%", "")) / 100.0 + 2 * float(skill_data["捉浪"]["数值"]["每层伤害提升"][level_e].replace("%", "")) / 100.0,
    ...
    if data["名称"] == "宵宫":
        return {
            "B:l50-增伤-*": (0.2, "被动一满层"),
            "B:c2-增伤-*": (0.25, "二命触发"),
            "A-e火-n2:普攻第一段": float(skill_data["焰硝庭火舞"]["数值"]["炽焰箭伤害"][level_e].replace("%普通攻击伤害", ""))
            / 100.0
            * float(skill_data["普通攻击·烟火打扬"]["数值"]["一段伤害"][level_a].replace("%*2", ""))
            / 100.0,
            "A-e火-r蒸发1.5:普攻第三段蒸发": (float(skill_data["焰硝庭火舞"]["数值"]["炽焰箭伤害"][level_e].replace("%普通攻击伤害", "")) / 100.0)
    ...
    if data["名称"] == "班尼特":
        ...
        return {
            ...
            "T:大招持续治疗": int((float(hp_recover[0].replace("每秒", "").replace("%生命值上限", "")) / 100.0 * health + int(hp_recover[1])) * (1 + data["属性"]["治疗加成"])),
        }
    if data["名称"] == "温迪":
        up = data["属性"]["扩散系数"] if "扩散系数" in data["属性"] else 0
        return {
    ...
    if data["名称"] == "琴":
        ...
        return {
            ...
            "T:大招持续治疗": int((float(recovery2[0].replace("每秒", "").replace("%攻击力", "")) / 100.0 * attack + int(recovery2[1])) * (1 + data["属性"]["治疗加成"])),
        }
    if data["名称"] == "七七":
