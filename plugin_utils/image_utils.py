...
ModeType = Literal["1", "CMYK", "F", "HSV", "I", "L", "LAB", "P", "RGB", "RGBA", "RGBX", "YCbCr"]
        ...
        if not background:
            ...
            if not w and not h:
                ...
                if ratio and ratio > 0 and ratio != 1:
                    self.w = int(ratio * w)
                    self.h = int(ratio * h)
                    self.markImg = self.markImg.resize((self.w, self.h), Image.Resampling.LANCZOS)
                else:
                    self.w = w
                    self.h = h
            else:
                self.markImg = Image.open(background).resize((self.w, self.h), Image.Resampling.LANCZOS)
        if is_alpha:
            try:
