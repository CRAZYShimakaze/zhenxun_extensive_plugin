...
forward = 0
download = 0
size_f = "regular"  # original large regular small thumb
re_list = ["i.pixiv.cat", "i.pixiv.re", "i.pixiv.nl"]  # 需要替换的域名列表
...
async def download_and_send(bot, event, mes_list, i):
    ...
    for rel in re_list:
        try:
            ...
            url_ = url_.replace(host_match.group(1), rel)
            # if download:
            ...
            if forward:
                data_info = {
                    "type": "node",
                    "data": {
                        "name": f"{event.sender.card or event.sender.nickname}",
                        "uin": f"{event.user_id}",
                ...
                # await check_gold(event, coin=100, percent=1)
...
async def se(bot, event, arg: Message = CommandArg()):
    ...
    if event.user_id != 674015283:
        r18 = 0
    num, tags = (
        tags[-1] if tags[-1].isdigit() else 1,
        tags[:-1] if tags[-1].isdigit() else tags,
    )
    if int(num) > 2:
        forward = 1
    else:
    ...
    if response.status_code == 200:
        ...
        if not data["error"]:
            ...
            if forward:
                # await check_gold(event, coin=100 * len(data), percent=1)
