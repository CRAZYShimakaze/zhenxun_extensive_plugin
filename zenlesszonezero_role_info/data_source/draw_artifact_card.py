...
from ..utils.card_utils import bg_path, char_pic_path, get_font, other_path, qq_logo_path, reli_path
from ..utils.image_utils import draw_center_text, draw_right_text, get_img, image_build, load_image
from .draw_role_card import integer_property, resource_url
...
async def draw_artifact_card(title, uid, artifact_info, ace2_num, ace_num, plugin_version, is_group=0):
    ...
    bg = load_image(f"{bg_path}/bg.png", size=(wid, hei), mode="RGBA")
    ...
    for index, artifact in enumerate(artifact_pk):
        ...
        if artifact["主属性"]["属性名"] not in integer_property:
            bg_draw.text((slice_offset_x + 20, slice_offset_y + 165), f"+{math.floor(artifact['主属性']['属性值']) / 100}%", fill="white", font=get_font(48, "number.ttf"))
        else:
    ...
    draw_center_text(bg_draw, f"{'group' if is_group else 'uid'}:{uid} | v{plugin_version} | Powered by Enka.Network", 0, wid, bg.size[1] - 70, "#ffffff", get_font(46, "优设标题黑.ttf"))
    text_info = "" if is_group else f"\n大毕业驱动盘{ace2_num}个,小毕业驱动盘{ace_num}个.\n最高评分为{artifact_pk[0]['角色']}{round(artifact_pk[0]['评分'], 2)}分的{artifact_pk[0]['名称']}!"
